...
class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        // Set categories
        $categoryEnum = ['Salle de réunion', 'Salle de formation', 'Amphithéâtre', 'Salle de spectacle', 'Salle polyvalante', 'Gymnase', 'Sans catégorie'];
        $categories = [];
        ...
        for ($i = 0; $i < 30; $i++) {
            $feature = new Feature();
            $description = $faker->text($faker->numberBetween(10, 40));
            $feature->setName(str_replace('.', '', $faker->text($faker->numberBetween(10, 20))))
                    ->setType($faker->randomElement(['Ergonomie', 'Matériel', 'Logiciel', 'Extras']))
